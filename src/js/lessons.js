...
Conditions use comparison operators like <code>>=</code>, <code>==</code>, <code><</code>, etc. The code inside each block must be indented (just like Python).
...
<strong>Fixed-count loops</strong> use <code>Run X mein:</code> to execute a block exactly X times. This is perfect when you know exactly how many iterations you need.

<strong>Conditional loops</strong> use <code>jab_tak</code> (while) to repeat as long as a condition is true. This is useful when you don't know how many iterations you'll need in advance.
...
• Functions without <code>give</code> return nothing (void)
...
func greet(name):
    out("Namaste,", name)
    give  # optional give with no value
greet("Riya")`,
    output: `Result: 15
Namaste, Riya`
  },
...
• Loops (<code>jab_tak</code>)
• Variables and user interaction

The game generates a random number between 1 and 20, then asks the player to guess it. After each guess, it provides feedback ("too low" or "too high") until the player guesses correctly.

This is a preview of how RID can handle more complex programs. Future versions of RID IDE will support interactive input directly in the editor!`,
    code: `# guesser.rid
import random
...
func get_guess():
    out("Enter your guess (1-20):")
    # assuming a built-in 'in()' in future; placeholder:
    g = int(input())    # transpiler must map input semantics
    give g
